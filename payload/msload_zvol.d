...
dtrace:::BEGIN
{
	...
	printf("Monitoring for metaslab loads\n");
	printf("Monitoring for requests slower than %d ms\n",
	    threshold_ms);
}

fbt::zvol_write:entry,
fbt::zvol_read:entry
{
	self->ts = timestamp;
}

fbt::zvol_write:return,
fbt::zvol_read:return
...
{
	apd = 1;
	printf("%Y BEGIN SLOW Response EVENT\n", walltimestamp);
}
...
/ apd == 1 && slow == 0 /
{
	...
	printa("END   SLOW Response EVENT - %@d reqs (max: %@dms, avg: %@dms)\n",
