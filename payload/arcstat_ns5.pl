...
my %cols = (# HDR => [Size, Scale, Description]
	...
	"cmpsz"	        =>[5, 1024, "Compressed ARC Size"],
	"uncmpsz"	=>[5, 1024, "Uncompressed ARC Size"],
	"ovh"		=>[4, 1024, "Overhead"],
...
my %v=();
my @hdr = qw(time read miss miss% dmis dm% pmis pm% mmis mm% arcsz cmpsz uncmpsz ovh c);
...
sub calculate {
	...
	$v{"arcsz"} = $cur{"size"};
	$v{"cmpsz"} = $cur{"compressed_size"};
	$v{"uncmpsz"} = $cur{"uncompressed_size"};
	$v{"ovh"} = $cur{"overhead"};
	$v{"c"} = $cur{"c"};
